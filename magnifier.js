;(function($){
  var Magnifier = function(opt) {
      ...
      $(document).mousemove(function(event) {
         var _self=this;
         var x=event.pageX-$(self.config.left_fixed_box).offset().left-self.config.mouse_width/2;
         var y=event.pageY-$(self.config.left_fixed_box).offset().top-self.config.mouse_height/2;
         self.setPos(x,y,event.pageX,event.pageY,$(self.config.left_fixed_box).offset().left,$(self.config.left_fixed_box).offset().top)
      })
    }
Magnifier.prototype.render=function(){
  var self=this;
  //生成节点
  $(self.config.left_fixed_box).append("<img src='"+self.config.small_img_src+"' class='small'>");
  $(self.config.left_fixed_box).append("<span style='display:none;'></span>");
  var $bigWrap=$("<div class='bigWrap' style='display:none;'></div>");
  var $photo=$("<div class='bigPhoto'></div>");
  $(self.config.left_fixed_box).append($bigWrap.html($photo));
...
Magnifier.prototype.setPos=function(x,y,pageX,pageY,l,t){
  if (pageX<l || (l+this.config.small_width)<pageX) {
    $(this.config.left_fixed_box).find("span").hide();
    $(this.config.left_fixed_box).find(".bigWrap").hide();
    return;
  }
  ...
  $(this.config.left_fixed_box).find("span").show();
  $(this.config.left_fixed_box).find(".bigWrap").show();
    if(x<0){
      x=0;
    }
    if(y<0){
    ...
    $(this.config.left_fixed_box).find("span").css({
      left: x,
      top: y
    });
    ...
    $(this.config.left_fixed_box).find(".bigPhoto").css({
